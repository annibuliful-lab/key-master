...
const validateWsAuthentication =
  (shouldSendForbiddenCode: boolean) => async (headers: IGatewayContext) => {
  ...
  const serverCleanup = useServer(
    {
      ...
      onConnect: async (
        ...
        return await validateWsAuthentication(false)(headers);
      },
      onSubscribe: async (ctx) => {
        const headers = ctx.connectionParams as unknown as IGatewayContext;
        await validateWsAuthentication(true)(headers);
      },
    },
    wsServer
  );
